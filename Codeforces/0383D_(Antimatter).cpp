...
int main()
{
    ...
    vector<int> a(n);
    for (int &e : a) cin >> e;
    vector<vector<ll>> dp(n + 1, vector<ll>(20002, 0LL));
    const int magic = 10000;
    dp[1][magic + a[0]] = dp[1][magic - a[0]] = 1LL;
    ll ans = 0LL;
    for (int i = 2; i <= n; ++i) {
        dp[i][magic + a[i - 1]] = dp[i][magic - a[i - 1]] = 1LL;
        for (int j = -magic + a[i - 1] ; j <= magic - a[i - 1]; ++j) {
            dp[i][magic + j] += dp[i - 1][magic + j - a[i - 1]];
            dp[i][magic + j] += dp[i - 1][magic + j + a[i - 1]];
            dp[i][magic + j] %= MOD;
        }
        ans += dp[i][magic];
