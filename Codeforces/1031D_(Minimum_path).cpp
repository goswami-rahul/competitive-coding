...
int main()
{
    ...
    for (int i = 1; i < n; ++i) {
        ...
        if (dp[i][0] <= k && i + 1 > maxa) maxa = i + 1;
        if (dp[0][i] <= k && i + 1 > maxa) maxa = i + 1;
    }
    for (int i = 1; i < n; ++i) {
        for (int j = 1; j < n; ++j) {
            dp[i][j] = (a[i][j] > 'a');
            dp[i][j] += min(dp[i - 1][j], dp[i][j - 1]);
            if (dp[i][j] <= k && i + j + 1 > maxa) maxa = i + j + 1;
        }
    }
    error(maxa)
    string ans(maxa, 'a');
    set<pair<char, pair<int,int>>> s, ns;
    if (maxa == 0) {
        s.emplace(a[0][0], mp(0, 0));
    }
    else {
        char minch = 'z';
        for (int i = 0; i < n; ++i) {
           for (int j = 0; j < n; ++j) {
               if (dp[i][j] == k && i + j + 1 == maxa) {
                    for (int d : {1, 0}) {
                        int x = i + d, y = j + 1 - d;
                        if (x >= n || y >= n) continue;
                        if (a[x][y] <= minch) {
                            minch = a[x][y];
                            s.emplace(minch, mp(x, y));
                        }
                    }
                }
            }
    ...
    while (!s.empty()) {
        char cur = s.begin()->first;
        ans.push_back(cur);
        char minch = 'z';
        ns.clear();
        for (auto &e : s) {
            if (e.first > cur) break;
            int i, j;
            tie(i, j) = e.second;
            for (int d : {1, 0}) {
                int x = i + d, y = j + 1 - d;
