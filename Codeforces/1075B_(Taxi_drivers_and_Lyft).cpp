...
int main()
{
    ...
    int n, m;
    cin >> n >> m;
    vector<int> temp(n + m), a, b, ans(m, 0);
    for (int i = 0; i < n + m; ++i) cin >> temp[i];
    for (int i = 0; i < n + m; ++i) {
        int x;
        cin >> x;
        if (x == 1) b.push_back(temp[i]);
        else a.push_back(temp[i]);
    }
    for (int i = 0; i < n; ++i) {
        int pos = (int) (lower_bound(all(b), a[i]) - b.begin());
        if (pos == m) {
            ans[m - 1]++;
        } else if (pos == 0) {
            ans[0]++;
