...
int main()
{
    ...
    int n;
    cin >> n;
    int xmax = -MOD, xmin = MOD, ymax = -MOD, ymin = MOD;
    set<pair<int,int>> p;
    for (int i = 0; i < n; ++i) {
        int x, y;
        cin >> x >> y;
    ...
    if (p.count({xmax, ymax}) || p.count({xmax, ymin}) || p.count({xmin, ymax}) || p.count({xmin, ymin})) {
        cout << ans << " ";
    } else {
        int k = -1;
        for (auto &pr: p) {
            int x = pr.first, y = pr.second;
            k = max(k, xmax - x + ymax - y);
            k = max(k, xmax - x + y - ymin);
            k = max(k, x - xmin + ymax - y);
            k = max(k, x - xmin + y - ymin);
        }
        cout << k * 2 << " ";
    }
    for (int i = 4; i <= n; ++i) {
