...
int main()
{
    ...
    for (auto &s: sub) sort(all(s));
    for (auto &s: sub) reverse(all(s));
    vector<int> idx(m);
    iota(all(idx), 0);
    sort(all(idx), [&] (int i, int j) {
        return (int) sub[i].size() > (int) sub[j].size();
    });
    for (auto &s: sub) {
    ...
    int t = (int) sub[idx[0]].size();
    i64 ans = 0;
    for (int p = 0; p < t; ++p) {
        i64 cur = 0;
        for (int ii = 0; ii < m; ++ii) {
            int i = idx[ii];
            if ((int) sub[i].size() <= p) break;
            if (sub[i][p] > 0) cur += sub[i][p];
        }
        ans = max(ans, cur);
    }
    cout << ans << '\n';
