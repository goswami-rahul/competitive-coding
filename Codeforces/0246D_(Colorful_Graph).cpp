...
int main()
{
    ...
    while (m--) {
        ...
        s[col[u]].insert(col[v]);
        s[col[v]].insert(col[u]);
        ans = max(ans, make_pair((int) s[col[u]].size(), -col[u]));
        ans = max(ans, make_pair((int) s[col[v]].size(), -col[v]));
    }
    cout << -ans.second;
    return 0;
}
