/*
problem required to find the probablity that x * y > S;
So, we need to draw the curve xy = S, find the area bounded
by it and the the the given boundaries, and divide it by the total
area to get the probabilty.
to get the area, integrate the function y = 1/x over the bounds.
*/
#include<bits/stdc++.h>
...
int main()
{
    ...
    while (n--) {
        ...
        long double ans, area = s * (1 + log(a) + log(b) - log(s));
        if (s < EPS) area = 0;
        ans = 1 - area / (a * b);
        ans *= 100;
        cout << ans << "%\n";
