/**
 * If there is no starting move, then Second wins.
 * Otherwise, the parity of length the resulting final string is unique.
 * Now, it will be even if final and last characters are different,
 * otherwise odd.
**/
#include <bits/stdc++.h>
using namespace std;
...
int32_t main() {
  ...
  for (int i = 1; i < n - 1; ++i)
    ok |= s[i - 1] != s[i + 1];
  int finish = 2 + (n > 2 && s[0] == s[n - 1]);
  kek(ok && (n - finish) & 1 ? "First" : "Second");
